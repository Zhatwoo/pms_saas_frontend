...
/**
 * Employee Pawn Renewal Slip modal — loads Settings `document_designs.renewal`
 * and fills fields from the renew transaction (no sample hardcoding).
 */

import { useEffect, useMemo, useRef, useState, type ReactNode } from "react";
import { MOA_PAGE_SIZES } from "@/app/(pages)/settings/_components/moa-design-palette";
import { api } from "@/lib/api";
import { setInterestRatesCache, type InterestRateGroup } from "@/lib/interest";
...
} from "@/lib/moa/build-renewal-slip-values";
...
export type RenewalSlipModalProps = {
  isOpen: boolean;
  onClose: () => void;
  /** Continue renew flow (e.g. open proof capture). */
  onConfirm: () => void;
  source: RenewalSlipSource | null;
  isConfirming?: boolean;
};

export function RenewalSlipModal({
  isOpen,
  onClose,
  onConfirm,
  source,
  isConfirming = false,
}: RenewalSlipModalProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setDesign(resolveEmployeeDocumentDesign(moaTemplate, "renewal", category));
      } catch (err) {
        console.error("Failed to load Pawn Renewal Slip template:", err);
        if (!cancelled && fetchId === fetchRef.current) {
          setLoadError("Could not load renewal slip template.");
          setDesign(resolveEmployeeDocumentDesign(null, "renewal", category));
        }
      } finally {
        if (!cancelled && fetchId === fetchRef.current) {
          setIsLoading(false);
  ...
  const fieldValues = useMemo(() => {
    ...
    return buildRenewalSlipFieldValues({
      ...source,
      shop: {
  ...
  const printableDesign = useMemo(() => {
    ...
    return applyRenewalPaymentTableAmounts(design, fieldValues);
  }, [design, fieldValues]);
      ...
      >
        <div className="flex shrink-0 items-center justify-between gap-3 bg-gradient-to-r from-brand-green via-brand-green to-brand-green px-5 py-4 text-white sm:px-6">
          <div className="min-w-0">
            ...
            <p className="mt-1.5 text-[10px] font-medium text-white/80 sm:text-[11px]">
              Review slip details from this renewal, then continue to process.
            </p>
          </div>
          <button
            type="button"
          ...
          {isLoading ? (
            ...
            <div className="flex min-h-[240px] flex-col items-center justify-center gap-2 px-6 py-16 text-center">
              <p className="text-sm font-bold text-zinc-700">
                {loadError || "No Pawn Renewal Slip design yet"}
              </p>
              <p className="max-w-sm text-[11px] font-medium text-zinc-500">
                Save a Pawn Renewal Slip under Settings → Slip Edit, then try again.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex shrink-0 flex-col gap-2 border-t border-zinc-200 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <p className="text-[10px] font-medium text-zinc-500">
            Layout from Settings · values from this renew transaction
          </p>
...
function RenewalSlipPreviewScale({
  children,
  pageCount,
  pageWidthPx,
  pageHeightPx,
