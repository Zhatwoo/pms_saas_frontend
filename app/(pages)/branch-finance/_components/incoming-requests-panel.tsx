  ...
  return (
    <div 
      className={`flex flex-col overflow-hidden rounded-xl border shadow-sm transition-all duration-300 ${
        expanded 
          ? "border-blue-500/50 bg-blue-500/5 ring-4 ring-blue-500/10 dark:ring-blue-500/5" 
          : "border-border-subtle bg-surface hover:border-blue-300 dark:hover:border-blue-500/40 hover:bg-blue-50/50 dark:hover:bg-blue-500/10"
      }`}
    >
      {/* Header (Clickable) */}
      <button
      ...
      >
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-400 shadow-inner">
            ...
            <span className="absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full border-2 border-surface bg-blue-600 text-[10px] font-bold text-white shadow-sm">
              {count}
            </span>
          </div>
        ...
        <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-50 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400 transition-transform duration-300" style={{ transform: expanded ? "rotate(180deg)" : "rotate(0deg)" }}>
          ...
          <div className="border-t border-blue-500/20 px-5 pb-5 pt-3">
            <div className="space-y-3">
              {requests.map((req) => (
                <div
                  key={req.id}
                  className="flex flex-col gap-4 rounded-xl border border-blue-200 dark:border-blue-500/20 bg-surface p-4 shadow-sm transition-all hover:shadow-md sm:h-[136px] sm:flex-row sm:items-center sm:justify-between relative overflow-hidden flex-shrink-0"
                >
                {/* Info side */}
                <div className="flex items-center gap-4">
                  <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-blue-50 dark:bg-blue-500/10 ring-2 ring-blue-100 dark:ring-blue-500/20">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-blue-600 dark:text-blue-400">
                  ...
                  <div>
                    ...
                    <div className="mt-0.5 flex items-center gap-2 text-sm font-medium text-text-secondary">
                      <span className="text-xl font-extrabold text-blue-700 dark:text-blue-400 tracking-tight">
                        {formatPeso(req.amount.toLocaleString("en-PH", { minimumFractionDigits: 2, maximumFractionDigits: 2 }))}
                      </span>
