...
export function RejectRequestModal({ isOpen, onClose, onConfirm, amount, branchName }: RejectRequestModalProps) {
      ...
      <div className="w-full max-w-md overflow-hidden rounded-2xl bg-surface shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between border-b border-border-main px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-text-primary">Reject Fund Request</h2>
              <p className="text-xs text-text-secondary">Provide a reason for rejection</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5">
          <div className="mb-6 rounded-xl border border-red-500/20 bg-red-50/50 p-4 text-center">
            <p className="text-xs font-semibold tracking-wider text-red-600">
