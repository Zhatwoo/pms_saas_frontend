...
export const SYSTEM_BENEFITS_ITEMS = [
  {
    id: "customer-management",
    title: "Customer Management",
    desc: "Create and maintain organized customer profiles and transaction history.",
    category: "Core Module",
  },
  {
    id: "pawn-transactions",
    title: "Pawn Transactions",
    desc: "Record and manage pawn transactions through a structured workflow.",
    category: "Core Module",
  },
  {
    id: "item-appraisal",
    title: "Item and Appraisal Management",
    desc: "Keep item descriptions, values, conditions, and transaction information connected.",
    category: "Inventory",
  },
  {
    id: "loans-payments",
    title: "Loans and Payments",
    desc: "Monitor loan balances, payments, interest, charges, and important transaction details.",
    category: "Financials",
  },
  {
    id: "renewals",
    title: "Renewals",
    desc: "Process and monitor loan renewals while keeping transaction records organized.",
    category: "Lifecycle",
  },
  {
    id: "redemptions",
    title: "Redemptions",
    desc: "Manage redeemed items and maintain complete redemption records.",
    category: "Lifecycle",
  },
  {
    id: "reports",
    title: "Reports",
    desc: "Turn your transaction data into useful business information.",
    category: "Analytics",
  },
  {
    id: "multi-branch",
    title: "Multi-Branch Management",
    desc: "Monitor multiple branches from a centralized platform.",
    category: "Enterprise",
  },
  {
    id: "printable-documents",
    title: "Printable Documents",
    desc: "Generate important transaction documents directly from the system.",
    category: "Operations",
  },
];

export function LandingBenefitsModal({
  isOpen,
  onClose,
  onScrollToContact,
}: {
  ...
  return (
    ...
    >
      <div
        ...
        className="relative max-h-[88vh] w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl transition-all duration-300"
      ...
      >
        ...
        <div className="relative overflow-hidden bg-brand-green px-6 pb-6 pt-7 text-white sm:px-8">
          ...
          <button
            type="button"
            onClick={onClose}
            aria-label="Close benefits modal"
          ...
          <div className="relative">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-brand-gold">{BRAND_CONFIG.shortCompanyName} Benefits & Features</p>
            <h3 id="benefits-modal-title" className="font-display mt-2 text-2xl font-bold md:text-3xl text-white">
              Everything Your Pawnshop Needs. In One System.
            </h3>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/80">
              QuickPawn brings essential pawnshop processes into a single structured platform — helping your team stay organized and focus on growing your business.
            </p>
          </div>
        </div>

        {/* Content Body */}
        <div className="max-h-[58vh] overflow-y-auto bg-[#f8f7f4] px-6 py-6 sm:px-8">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {SYSTEM_BENEFITS_ITEMS.map((item, idx) => (
              <div
                key={item.id}
                className="group flex flex-col justify-between rounded-2xl border border-brand-green/10 bg-white p-5 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-brand-gold/50 hover:shadow-md"
              >
                <div>
                  <div className="flex items-center justify-between">
                    <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand-green/10 text-xs font-black text-brand-green group-hover:bg-brand-green group-hover:text-brand-gold transition-colors">
                      0{idx + 1}
                    </span>
                    <span className="rounded-full bg-brand-gold/15 px-2.5 py-0.5 text-[10px] font-bold text-brand-green">
                      {item.category}
                    </span>
                  </div>
                  <h4 className="font-display mt-3 text-base font-bold text-brand-green">{item.title}</h4>
                  <p className="mt-1.5 text-xs leading-relaxed text-zinc-600">{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Footer */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-zinc-200 bg-white px-6 py-4 sm:px-8">
          <p className="text-xs font-semibold text-zinc-500 text-center sm:text-left">
            Experience how QuickPawn transforms pawnshop operations.
          </p>
          <div className="flex items-center gap-3 w-full sm:w-auto">
            {onScrollToContact ? (
              <button
                ...
                className="flex-1 sm:flex-initial rounded-full bg-brand-green px-5 py-2.5 text-xs font-black uppercase tracking-wider text-white transition hover:bg-brand-green/90 text-center"
              >
                Schedule a Demo
              </button>
            ) : null}
            <button
              type="button"
              onClick={onClose}
              className="rounded-full border border-zinc-200 bg-zinc-100 px-4 py-2.5 text-xs font-bold text-zinc-700 transition hover:bg-zinc-200"
            >
              Close
            </button>
          </div>
...
export function LandingBenefits({ onScrollToContact }: { onScrollToContact?: () => void }) {
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="reveal-on-scroll mt-12 text-center">
          <button
            ...
            className="inline-flex items-center gap-2.5 rounded-full bg-brand-green px-7 py-3.5 text-xs font-black uppercase tracking-wider text-white shadow-xl shadow-brand-green/20 transition-all duration-300 hover:bg-brand-green/90 hover:scale-105"
          >
            <span>Everything Your Pawnshop Needs. In One System.</span>
            <svg className="h-4 w-4 text-brand-gold" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}>
