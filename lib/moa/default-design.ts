 ...
 *  Name/Date lines are bound fields so they auto-fill from New Pawn data. */
function signatures(
  prefix: string,
  y: number,
  pageIndex: number,
  ...
  const block = (
    ...
    return [
      ...
      cfield(prefix, "purchasedDate", "Date", x, top + 82, 356, pageIndex),
    ];
  };
...
export function createGeneralMoaDesign(): MoaDesignBlob {
  ...
  const page0: MoaDesignElement[] = [
    ...shopHeader(p),
    title(p, "title", "MEMORANDUM OF AGREEMENT", 112, 0, 20, 30),
    heading(p, "subtitle", "PAWN LOAN AGREEMENT", 146, 0, 11),
    cfield(p, "agreementNo", "Agreement No.", 24, 172, 360),
    para(
      p,
      "intro",
      'This Memorandum of Agreement ("Agreement") is entered into on the ____ day of ____________, 20____, by and between:',
      206,
      34,
    ),
    heading(p, "pawnshop-label", "PAWNSHOP", 246, 0, 11, "center"),
    cfield(p, "shopName", "Business Name", 24, 270, 736),
    cfield(p, "representedBy", "Represented by", 24, 304, 736),
    para(p, "pawnshop-note", 'Hereinafter referred to as the "Pawnshop";', 338, 18),
    heading(p, "and", "- and -", 360, 0, 11, "center"),
    heading(p, "customer-label", "CUSTOMER / PAWNER", 384, 0, 11, "center"),
    cfield(p, "customerName", "Full Name", 24, 408, 736),
    cfield(p, "customerAddress", "Address", 24, 442, 736),
    cfield(p, "contactNo", "Contact Number", 24, 476, 360),
    cfield(p, "idPresented", "Government ID Presented", 400, 476, 360),
    cfield(p, "idNumber", "ID Number", 24, 510, 360),
    para(
      p,
      "customer-note",
    ...
    heading(p, "s2", "II. DESCRIPTION OF PAWNED ITEM", 644),
    cfield(p, "pawnTicketNo", "Pawn Ticket No.", 24, 668, 360),
    cfield(p, "brandModel", "Item Description", 400, 668, 360),
    cfield(p, "brand", "Brand", 24, 702, 360),
    cfield(p, "model", "Model", 400, 702, 360),
    cfield(p, "serialNo", "Serial Number", 24, 736, 360),
    cfield(p, "itemsIncluded", "Accessories Included", 400, 736, 360),
    cfield(p, "condition", "Condition upon acceptance", 24, 770, 360),
    cfield(p, "appraisedValue", "Declared / Appraised Value (PHP)", 400, 770, 360),
    heading(p, "s3", "III. LOAN DETAILS", 812),
    cfield(p, "amount", "Loan Amount (PHP)", 24, 836, 360),
    cfield(p, "interestRate", "Interest Rate (% per)", 400, 836, 360),
    cfield(p, "storageFee", "Service Charges (PHP)", 24, 870, 360),
    cfield(p, "purchasedDate", "Date Pawned", 400, 870, 360),
    cfield(p, "maturityDate", "Maturity Date", 24, 904, 360),
    cfield(p, "expiryDate", "Expiry Date", 400, 904, 360),
    cfield(p, "netProceeds", "Total Amount for Redemption (PHP)", 24, 938, 736),
    heading(p, "s4", "IV. CUSTOMER DECLARATIONS", 980),
    para(
      p,
      "s4-body",
  ...
  const page1: MoaDesignElement[] = [
    heading(p, "s5", "V. REDEMPTION", 20, 1),
    para(
      p,
      "s5-body",
    ...
    heading(p, "s7", "VII. DEFAULT", 164, 1),
    para(
      p,
      "s7-body",
    ...
    heading(p, "s8", "VIII. CUSTOMER RESPONSIBILITY", 232, 1),
    para(
      p,
      "s8-body",
    ...
    heading(p, "s9", "IX. DATA PRIVACY ACT", 304, 1),
    para(
      p,
      "s9-body",
    ...
    heading(p, "s10", "X. GOVERNING LAW", 402, 1),
    para(
      p,
      "s10-body",
    ...
    heading(p, "s11", "XI. ENTIRE AGREEMENT", 454, 1),
    para(
      p,
      "s11-body",
    ...
    ...signatures(p, 540, 1, { rep: true, witness: true }),
    footer(p, 830, 1),
  ];
